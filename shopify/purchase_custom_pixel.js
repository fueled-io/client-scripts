const services = {
  facebook: {
    ...
    async format(event) {
      ...
      const rawValue = this.config.valueField === "total" ? checkoutData?.totalPrice?.amount : checkoutData?.subtotalPrice?.amount;
      ...
      return {
        ...
        value,
        order_id: utils.shopifyGidToId(checkoutData?.order?.id) || checkoutData?.token,
    ...
    async handler (event) {
      ...
      const checkoutData = event?.data?.checkout || event?.customData?.checkout;
      const eventId = await utils.getEventId(payload.order_id || checkoutData?.token);
      ...
      for (const type of additionalEventTypes) {
        const orderId = payload.order_id || checkoutData?.token;
        windowFbq("trackCustom", type, payload, {
          eventID: await utils.getEventId([orderId, type].join("_"))
        });
      }
...
const utils = {
  ...
  async getExternalId() {
    const userToken = await this.readCookie("fueledUserToken");
    if (userToken) {
      return userToken;
    }
